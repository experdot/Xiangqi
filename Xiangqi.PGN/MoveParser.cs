...
namespace Xiangqi.PGN
{
	...
	private static short[,] FIX_MOVE = {
		...
		private static int char2Piece(int c)
		{
			if (c >= 'a' && c <= 'z')
			{
				return c == 'e' ? 2 : c == 'h' ? 3 : PIECE_TO_CHAR.IndexOf((char)(c - 'a' + 'A'));
			}
		...
		private static int word2Digit(int w)
		{
			...
			{
				int index = DIGIT_TO_WORD[i].IndexOf((char)w);
				if (index >= 0)
				{
					return index;
				}
		...
		private static int word2Piece(int w)
		{
			...
			{
				int index = PIECE_TO_WORD[i].IndexOf((char)w);
				if (index >= 0)
				{
					return index;
				}
		...
		private static int word2Direct(int w)
		{
			...
			{
				int index = DIRECT_TO_WORD[i].IndexOf((char)w);
				if (index >= 0)
				{
					return index;
				}
		...
		private static int word2Pos(int w)
		{
			...
			{
				int index = POS_TO_WORD[i].IndexOf((char)w);
				if (index >= 0)
				{
					return index;
				}
